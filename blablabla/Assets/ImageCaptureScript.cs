...
public class ImageCaptureScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        OnCameraChange();
        OnSceneChange();
        time = Time.realtimeSinceStartup * 1000000;
        var passfilename = Path.GetFileNameWithoutExtension(filename + time);
        //Save(filename + time, width: (int)gameViewSize.x, height: (int)gameViewSize.y, filepath);
        Save(passfilename, width: Screen.width, height: Screen.height, filepath);
    }
